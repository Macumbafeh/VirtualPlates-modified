...
local FRIENDLY_MULTIPLIER = 0.75
			...
			function PlatesUpdate ()
				...
				if ( #SortOrder > 0 ) then
					...
					for Index, Plate in ipairs( SortOrder ) do
						...
						local reaction, type = "HOSTILE", "PLAYER"  -- full size by default

						if Plate.healthBar and Plate.healthBar.GetStatusBarColor then
							local r, g, b, a = Plate.healthBar:GetStatusBarColor()
							reaction, type = reactionByColor(r, g, b, a)
 						end
						...
						if reaction == "FRIENDLY" then
							Scale = Scale * FRIENDLY_MULTIPLIER
						end
